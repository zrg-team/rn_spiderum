...
function handleItem (item) {
  ...
  if (item.creator_id && item.creator_id.avatar) {
    item.avatar = `${AVATAR_URL}${item.creator_id.avatar}`
  }
  const regex = /(<([^>]+)>)/ig
...
export function parseHtml (page, data) {
  ...
  database.model('article').delete({
    ...
    console.debug('[PARSE HTML] Error', error)
  })
...
export default (dispatch, props) => ({
  getNews: async (page, type) => {
    try {
      ...
      console.debug('[GET NEWS] Error', err)
      return undefined
    }
  },
  searchNews: async (page, params) => {
    try {
      ...
      console.debug('[SEARCH NEWS] Error', err)
      return undefined
    }
  }
})
